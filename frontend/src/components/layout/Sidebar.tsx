"use client";

import { useState } from "react";
import {
  Shield,
  LayoutDashboard,
  Link2,
  Mail,
  QrCode,
  Network,
  ScanEye,
  Search,
  Settings,
  ChevronLeft,
  ChevronRight,
  Activity,
} from "lucide-react";
import { cn } from "@/lib/utils";

export type ViewType =
  | "dashboard"
  | "url-scanner"
  | "email-analyzer"
  | "qr-scanner"
  | "network-monitor"
  | "deepfake-detector"
  | "threat-intel";

interface NavItem {
  id: ViewType;
  label: string;
  icon: React.ElementType;
}

const navItems: NavItem[] = [
  { id: "dashboard", label: "Dashboard", icon: LayoutDashboard },
  { id: "url-scanner", label: "Escáner URL", icon: Link2 },
  { id: "email-analyzer", label: "Analizador Email", icon: Mail },
  { id: "qr-scanner", label: "Escáner QR", icon: QrCode },
  { id: "network-monitor", label: "Monitor de Red", icon: Network },
  { id: "deepfake-detector", label: "Detector Deepfake", icon: ScanEye },
  { id: "threat-intel", label: "Inteligencia", icon: Search },
];

interface SidebarProps {
  activeView: ViewType;
  onViewChange: (view: ViewType) => void;
}

export default function Sidebar({ activeView, onViewChange }: SidebarProps) {
  const [collapsed, setCollapsed] = useState(false);
  const [healthOk, setHealthOk] = useState(true);

  return (
    <aside
      className={cn(
        "flex flex-col h-screen bg-[#0d1117] border-r border-[#1e293b] transition-all duration-300 relative z-20",
        collapsed ? "w-[68px]" : "w-[240px]"
      )}
    >
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 h-16 border-b border-[#1e293b]">
        <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-cyan-500/10 border border-cyan-500/30 shrink-0">
          <Shield className="w-5 h-5 text-cyan-400" />
        </div>
        {!collapsed && (
          <div className="overflow-hidden">
            <h1 className="text-sm font-bold text-white tracking-wide">
              AI-SENTINEL
            </h1>
            <p className="text-[10px] text-cyan-400/70 tracking-widest uppercase">
              Ciberseguridad IA
            </p>
          </div>
        )}
      </div>

      {/* Navigation */}
      <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
        {navItems.map((item) => {
          const isActive = activeView === item.id;
          const Icon = item.icon;
          return (
            <button
              key={item.id}
              onClick={() => onViewChange(item.id)}
              className={cn(
                "flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm transition-all duration-200 group",
                isActive
                  ? "bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 shadow-[0_0_10px_#06b6d415]"
                  : "text-slate-400 hover:text-slate-200 hover:bg-white/5 border border-transparent"
              )}
              title={collapsed ? item.label : undefined}
            >
              <Icon
                className={cn(
                  "w-[18px] h-[18px] shrink-0 transition-colors",
                  isActive ? "text-cyan-400" : "text-slate-500 group-hover:text-slate-300"
                )}
              />
              {!collapsed && (
                <span className="truncate">{item.label}</span>
              )}
              {isActive && !collapsed && (
                <div className="ml-auto w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse-dot" />
              )}
            </button>
          );
        })}
      </nav>

      {/* Bottom section */}
      <div className="border-t border-[#1e293b] p-2 space-y-1">
        <button
          className={cn(
            "flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm text-slate-400 hover:text-slate-200 hover:bg-white/5 transition-colors"
          )}
          title={collapsed ? "Configuración" : undefined}
        >
          <Settings className="w-[18px] h-[18px] shrink-0 text-slate-500" />
          {!collapsed && <span>Configuración</span>}
        </button>

        {/* Health indicator */}
        <div
          className={cn(
            "flex items-center gap-3 px-3 py-2 rounded-lg",
            collapsed ? "justify-center" : ""
          )}
        >
          <div className="relative shrink-0">
            <div
              className={cn(
                "w-2 h-2 rounded-full",
                healthOk ? "bg-emerald-400" : "bg-red-400"
              )}
            />
            <div
              className={cn(
                "absolute inset-0 w-2 h-2 rounded-full animate-ping",
                healthOk ? "bg-emerald-400" : "bg-red-400"
              )}
            />
          </div>
          {!collapsed && (
            <span className="text-xs text-slate-500">
              {healthOk ? "Sistema operativo" : "Sin conexión"}
            </span>
          )}
        </div>
      </div>

      {/* Collapse toggle */}
      <button
        onClick={() => setCollapsed(!collapsed)}
        className="absolute -right-3 top-20 w-6 h-6 rounded-full bg-[#1e293b] border border-[#334155] flex items-center justify-center text-slate-400 hover:text-white hover:bg-[#334155] transition-colors z-30"
      >
        {collapsed ? (
          <ChevronRight className="w-3 h-3" />
        ) : (
          <ChevronLeft className="w-3 h-3" />
        )}
      </button>
    </aside>
  );
}
